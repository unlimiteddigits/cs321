...
static GLfloat LR = -3.0, scale = 0.4, range = 1.0;
...
void draw_cube(int v1, int v2, int v3, int v4)
{
	...
	glTexCoord2f(0.0, range);		glVertex3fv(vertices + v2 * 3);
	glTexCoord2f(range, range);		glVertex3fv(vertices + v3 * 3);
	glTexCoord2f(range, 0.0);		glVertex3fv(vertices + v4 * 3);
	glEnd();
}
...
void keyboard(unsigned char key, int x, int y)
{
	...
	case '-':
		...
		break;
	case 'I': case 'i':
		range += 1.0;                  // SCALES THE texture on the object
		break;
	case 'D': case 'd':
		range -= 1.0;
		break;
	case 'Q':	case 'q': exit(2);
	}
	glutPostRedisplay();
}
